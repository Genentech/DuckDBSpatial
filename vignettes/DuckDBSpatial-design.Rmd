...
values are pulled. The DuckDB spatial extension is loaded lazily on the shared
connection (as an optional extension, so a build without it degrades rather than
fails).
...
```{r geoparquet, eval=requireNamespace("nanoparquet", quietly=TRUE)}
